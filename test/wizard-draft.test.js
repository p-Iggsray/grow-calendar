...
test("sanitize: a valid draft passes through with step intact", () => {
  const out = sanitizeWizardDraft(
    { survey: { growName: "Summer" }, step: 5 },
    STEP_COUNT
  );
  assert.deepEqual(out, { survey: { growName: "Summer" }, step: 5 });
});
...
test("sanitize: legacy drafts with extra fields (old wantTasks) still load", () => {
  const out = sanitizeWizardDraft({ survey: { a: 1 }, step: 2, wantTasks: true }, STEP_COUNT);
  assert.deepEqual(out, { survey: { a: 1 }, step: 2 });
});
...
test("save then load restores the draft for the same grow id", () => {
  const draft = { survey: { environment: "indoor", vegWeeks: 4 }, step: 5 };
...
test("clear removes the draft, and load of corrupted JSON returns null", () => {
  saveWizardDraft("g1", { survey: {}, step: 2 });
  clearWizardDraft("g1");
...
test("drafts are keyed per grow id and a missing id falls back to a shared key", () => {
  saveWizardDraft(null, { survey: { growName: "keyless" }, step: 1 });
