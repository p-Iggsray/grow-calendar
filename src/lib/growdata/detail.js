...
// The app shows no em or en dashes anywhere. Older AI-generated plans stored in
// the DB still contain them, so scrub every string that leaves getDetail.
function deDash(s) {
...
function scrubDetail(detail) {
  ...
  return {
    ...detail,
    title: deDash(detail.title),
    summary: deDash(detail.summary),
    notes: deDash(detail.notes),
    ...(detail.warning != null ? { warning: deDash(detail.warning) } : {}),
    tasks: (detail.tasks ?? []).map(deDash),
  };
}
...
export function getDetail(date, config, overrides, generatedPlan, phaseOverrides, eventRules = []) {
  ...
  return scrubDetail(applyDayOverride(base, dayOverride));
}
