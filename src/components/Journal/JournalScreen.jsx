...
import { ChevronLeft, ChevronRight, BookOpen, PenLine, Droplets, StickyNote, Sprout } from "lucide-react";
...
// One day of the grow journal: the daily log, the day note, and every plant's
// entries, with flip-through navigation and jump-to-any-day. `active` goes
// false while the DayView overlay covers the journal; flipping back to true
// refetches, so edits made in the overlay show up the moment it closes.
export default function JournalScreen({ today, date, onChangeDate, config, growId, onOpenDay, active = true }) {
  ...
  return (
    <div style={{ padding: "4px 14px 20px", display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <button
          ...
          <div style={{ fontFamily: UI, fontSize: 10.5, color: "var(--c-text-ghost)", marginTop: 2, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
            ...
            <span>Tap to jump to a date</span>
          </div>
        </button>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: active ? "rgba(34,197,94,0.14)" : "var(--c-surface-1)",
                  border: active ? "1px solid rgba(34,197,94,0.45)" : "1px solid var(--c-border-faint)",
                  cursor: "pointer",
                }}>
                <div style={{ fontFamily: NUM, fontSize: 13.5, color: active ? "var(--c-accent)" : "var(--c-text-dim)" }}>
                  {Number(k.slice(8, 10))}
                </div>
      ...
      {loading ? (
        ...
        <div className="card" style={{ padding: "34px 20px", textAlign: "center" }}>
          ...
          <div style={{ fontFamily: UI, fontSize: 14.5, fontWeight: 700, color: "var(--c-text)", marginTop: 10 }}>
            Nothing in the journal for this day
          </div>
          <div style={{ fontFamily: UI, fontSize: 12.5, color: "var(--c-text-muted)", marginTop: 6, lineHeight: 1.6 }}>
            The daily log, your day note, and any plant entries you record will all show up here.
          </div>
        </div>
      ) : (
        <>
          ...
          {day.note && (
            <Card title="Day note" icon={<StickyNote size={13} strokeWidth={2} style={{ color: "#60a5fa" }} />}>
              <div style={{ fontFamily: UI, fontSize: 13.5, color: "var(--c-text-dim)", lineHeight: 1.7, whiteSpace: "pre-wrap", overflowWrap: "break-word" }}>
                {day.note}
              </div>
            </Card>
          )}
                  ...
                  {g.entries.map(e => {
                    ...
                    return (
                      <div key={e.id} style={{ padding: "7px 0", borderTop: "1px solid var(--c-border-faint)" }}>
                        ...
                        {e.body && (
                          <div style={{ fontFamily: UI, fontSize: 12.5, color: "var(--c-text-dim)", lineHeight: 1.6, marginTop: 3, whiteSpace: "pre-wrap", overflowWrap: "break-word" }}>
                            {e.body}
                          </div>
                        )}
                      </div>
      ...
      {/* One tap into the full day view to add or edit anything shown here. */}
      <button
        type="button"
        className="touch-target"
        ...
        {isEmpty ? "Log this day" : "Add or edit this day"}
      </button>
    </div>
  );
}
